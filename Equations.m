function F=Equations(x,Az,Bz,Ar,Br,Nz,Nr,u0,C0,Pt,epsilon,Density_bed,Deffz,Deffr,keffz,keffr,R_nodes,kg,hg,as,Components,RxnKinetic,R)
...
for i=1:Nz
    for k=1:Nr 
        ...
        E_Cs_C2H4(i,k)= (1-epsilon)*kg*as*(C_C2H6(i,k) - Cs_C2H6(i,k)) + Density_bed*(ODHReactions(C_solid,Ts(i,k),R,Pt,Flowin,RxnKinetic,[Components.deltaS0],[Components.deltaH0],2,'Mass'));
        
        E_Cs_O2(i,k)=   (1-epsilon)*kg*as*(C_C2H6(i,k) - Cs_C2H6(i,k)) + Density_bed*(ODHReactions(C_solid,Ts(i,k),R,Pt,Flowin,RxnKinetic,[Components.deltaS0],[Components.deltaH0],3,'Mass'));
        
        E_Cs_CO2(i,k)=  (1-epsilon)*kg*as*(C_C2H6(i,k) - Cs_C2H6(i,k)) + Density_bed*(ODHReactions(C_solid,Ts(i,k),R,Pt,Flowin,RxnKinetic,[Components.deltaS0],[Components.deltaH0],4,'Mass'));
        
        E_Cs_CO(i,k)=   (1-epsilon)*kg*as*(C_C2H6(i,k) - Cs_C2H6(i,k)) + Density_bed*(ODHReactions(C_solid,Ts(i,k),R,Pt,Flowin,RxnKinetic,[Components.deltaS0],[Components.deltaH0],5,'Mass'));
        
        E_Cs_H2O(i,k)=  (1-epsilon)*kg*as*(C_C2H6(i,k) - Cs_C2H6(i,k)) + Density_bed*(ODHReactions(C_solid,Ts(i,k),R,Pt,Flowin,RxnKinetic,[Components.deltaS0],[Components.deltaH0],6,'Mass'));
        
        E_Cs_N2(i,k)=    (1-epsilon)*kg*as*(C_C2H6(i,k) - Cs_C2H6(i,k)); % Term of reaction must be ommited for N2 component, consider it's an inert component
        ...
        E_T(i,k)= -u0*Rof(i,k)*Cpf(i,k)*Az(i+1,2:end-1)*T(:,k) - u0*Rof(i,k)*Cpf(i,k)*Az(i+1,1)*(u0*Rof(i,k)*Cpf(i,k)*T0 + keffz*Az(1,2:end-1)*T(:,k) + keffz*Az(1,end)/Az(end,end)*(-Az(end,2:end-1)*T(:,k)) )/((1 + keffz*Az(end,1)*Az(1,end)/Az(end,end))*(u0*Rof(i,k)*Cpf(i,k) - keffz*Az(1,1))) -u0*Rof(i,k)*Cpf(i,k)*Az(i+1,end)*(Az(end,1:end-1)*T(2:end-1,k) - Az(end,1)*(u0*Rof(i,k)*Cpf(i,k)*T0 + keffz*Az(1,2:end-1)*T(:,k) + keffz*Az(1,end)/Az(end,end)*(-Az(end,2:end-1)*T(:,k)) )/((1 + keffz*Az(end,1)*Az(1,end)/Az(end,end))*(u0*Rof(i,k)*Cpf(i,k) - keffz*Az(1,1))) )/Az(end,end)...
            + keffz*Bz(i+1,2:end-1)*T(:,k) + keffz*Bz(i+1,1)*(u0*Rof(i,k)*Cpf(i,k)*T0 + keffz*Az(1,2:end-1)*T(:,k) + keffz*Az(1,end)/Az(end,end)*(-Az(end,2:end-1)*T(:,k)) )/((1 + keffz*Az(end,1)*Az(1,end)/Az(end,end))*(u0*Rof(i,k)*Cpf(i,k) - keffz*Az(1,1))) + keffz*Bz(i+1,end)*(Az(end,1:end-1)*T(2:end-1,k) - Az(end,1)*(u0*Rof(i,k)*Cpf(i,k)*T0 + keffz*Az(1,2:end-1)*T(:,k) + keffz*Az(1,end)/Az(end,end)*(-Az(end,2:end-1)*T(:,k)) )/((1 + keffz*Az(end,1)*Az(1,end)/Az(end,end))*(u0*Rof(i,k)*Cpf(i,k) - keffz*Az(1,1))) )/Az(end,end) + keffr/R_nodes(k+1)*Ar(k+1,2:end-1)*T(i,:) ...
            + keffr/R_nodes(k+1)*Ar(k+1,1)*(-Ar(1,2:end-1)*T(i,:) - keffr*Ar(1,end)*(Ar(end,2:end-1)*T(i,:) + hw*Tb)/(hw - keffr*Ar(end,end)))/((1 + (keffr*Ar(1,end)*Ar(end,1))/(hw - keffr*Ar(end,end)))*Ar(1,1)) + keffr/R_nodes(k+1)*Ar(k+1,end)*(keffr*Ar(end,2:end-1)*T(i,:) + hw*Tb + keffr*Ar(end,1)*(-Ar(1,2:end-1)*T(i,:) - keffr*Ar(1,end)*(Ar(end,2:end-1)*T(i,:) + hw*Tb)/(hw - keffr*Ar(end,end)))/((1 + (keffr*Ar(1,end)*Ar(end,1))/(hw - keffr*Ar(end,end)))*Ar(1,1)))/(hw - keffr*Ar(end,end)) + keffr*Br(k+1,2:end-1)*T(i,:) + keffr*Br(k+1,1)*(-Ar(1,2:end-1)*T(i,:) - keffr*Ar(1,end)*(Ar(end,2:end-1)*T(i,:) + hw*Tb)/(hw - keffr*Ar(end,end)))/((1 + (keffr*Ar(1,end)*Ar(end,1))/(hw - keffr*Ar(end,end)))*Ar(1,1)) ...
            + keffr*Br(k+1,end)*(keffr*Ar(end,2:end-1)*T(i,:) + hw*Tb + keffr*Ar(end,1)*(-Ar(1,2:end-1)*T(i,:) - keffr*Ar(1,end)*(Ar(end,2:end-1)*T(i,:) + hw*Tb)/(hw - keffr*Ar(end,end)))/((1 + (keffr*Ar(1,end)*Ar(end,1))/(hw - keffr*Ar(end,end)))*Ar(1,1)))/(hw - keffr*Ar(end,end)) + (1-epsilon)*hg*as*(Ts(i,k) - T(i,k));
